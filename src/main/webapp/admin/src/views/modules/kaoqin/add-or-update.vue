<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="上课地点" prop="shangkedidian">
          <el-select v-model="ruleForm.shangkedidian" placeholder="请选择上课地点">
            <el-option
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="上课日期" prop="shangkeriqi">
          <el-select v-model="ruleForm.shangkeriqi" placeholder="请选择上课日期">
            <el-option
                v-for="(item,index) in shangkeriqiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="上课日期" prop="shangkeriqi">
	      <el-input v-model="ruleForm.shangkeriqi"
                placeholder="上课日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.kaoqintupian" label="考勤图片" prop="kaoqintupian">
          <file-upload
          tip="点击上传考勤图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.kaoqintupian?ruleForm.kaoqintupian:''"
          @change="kaoqintupianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.kaoqintupian" label="考勤图片" prop="kaoqintupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.kaoqintupian.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shangkedidian = true;
          ...
          if(o=='shangkeriqi'){
            this.ruleForm.shangkeriqi = obj[o];
	    this.ro.shangkeriqi = true;
            continue;
          }
          if(o=='shangkeshijian'){
	    ...
	    this.ro.shangkeshijian = true;
          ...
          if(o=='kaoqintupian'){
            this.ruleForm.kaoqintupian = obj[o];
	    this.ro.kaoqintupian = true;
            continue;
          }
          if(o=='kaoqinshijian'){
      ...
      this.$http({
		...
		if(json.jiaoshixingming!=''&&json.jiaoshixingming){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/kechengbiao/kechengmingcheng`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
            ...
            this.$http({
              url: `option/kechengbiao/shangkedidian`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
            ...
            this.$http({
              url: `option/kechengbiao/shangkeriqi`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.shangkeriqiOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
            ...
            this.$http({
              url: `option/kechengbiao/shangkeshijian`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.shangkeshijianOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    kaoqintupianUploadChange(fileUrls) {
	this.ruleForm.kaoqintupian = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
