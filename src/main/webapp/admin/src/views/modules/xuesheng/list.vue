<template>
  <div class="main-content">
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('xuesheng','分配班级') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="banjiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'分配班级':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','分配班级') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="banjiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'分配班级':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','分配班级') && contents.tableBtnIcon == 0" type="success" size="mini" @click="banjiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'分配班级':'' }}</el-button>
    ...
    <banji-cross-add-or-update v-if="banjiCrossAddOrUpdateFlag" :parent="this" ref="banjiCrossaddOrUpdate"></banji-cross-add-or-update>
...
import banjiCrossAddOrUpdate from "../banji/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      banjiCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    banjiCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.banjiCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','xuesheng');
      this.$nextTick(() => {
      this.$refs.banjiCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
